...
开发执行者只提交候选知识点。主程审核候选知识点后，调用通用 wiki 或本机项目胶囊内的项目 wiki 维护规则判断是否沉淀，并记录写入或不写入的具体理由。
...
| 目标项目技术决策 | 本机项目胶囊内的目标项目 wiki |
| 目标项目踩坑经验 | 本机项目胶囊内的目标项目 wiki 或运行记录 |
| 一次性调试过程 | 运行记录 |
