...
title: Unity UGUI UI 技术美术拼装规则
workflow: unity-ugui-ui-workflow
phase: technical-art
read_when: 需要验收用户已切元素、整理正式资源、输出拼接还原文档。
updated: 2026-05-08
---

# Unity UGUI UI 技术美术拼装规则

技术美术负责把 UI 策划交接内容和用户提供的已切元素图片整理成 Unity 可施工资源包，并输出拼接还原文档。

## 角色声明

- 技术美术开始输出时，必须先显示 `角色：技术美术`。
- 技术美术只能处理已切元素验收、资源分类、命名、导入建议、拼接还原和施工输入，不进入 Unity C#、Prefab 生成器或业务按钮接线。
...
- 禁止把未切素材合集当作正式资源交给 Unity 施工员。
- 禁止把最终完整展示图作为背景图；只有用户提供的纯背景图可以作为背景层整图使用。

如果缺少某个必要元素，技术美术必须写入 `MissingAssetList`，并回报 UI 策划要求用户补充已切元素图。
...
2. 完整展示图，用于视觉对齐和验收。
3. 用户提供的已切元素图片。
4. 设计分辨率或 UI 策划确认的参考分辨率。

只有最终展示图时，技术美术不能产出正式 Unity 资源包，只能输出元素需求清单和缺失项。
...
- 技术美术不从最终展示图裁切资源，但可以根据完整展示图做第一版视觉估算坐标。
...
Unity 施工员只能使用 UI 策划审核通过、且技术美术标记为“可直接使用”的 `Final` 资源。`ReferenceOnly`、未切素材合集、污染元素和待补元素不得进入 Unity 施工依赖表。
