...
| `development` | 开发执行 | active | `../workflows/development/WORKFLOW.md` | 3.0.0 | 目标明确，需要主程按项目 wiki 策划页面拆解项目 wiki 开发任务页，并分配子任务给全新会话执行；项目专属入口由本机项目胶囊声明。 |
| `unity-ugui-ui-workflow` | Unity UGUI UI 图转施工工作流 | active | `../workflows/unity-ugui-ui-workflow/WORKFLOW.md` | 1.2.0 | 用户提供 UI 截图、效果图或素材图，需要按快速路径、资源核查路径或独立施工路径输出 UI 规划、资源验收、UGUI Prefab 施工、Controller 事件接口和截图校准反馈；会话首次正式输出和正式结论按会话角色标识规则展示。 |
| `game-development-role-routing` | 游戏开发角色路由 | active | `../workflows/development/rules/game-role-routing.md` | 3.0.0 | 需要按 UI、3C、场景、战斗、AI、玩法系统、工具、技术美术或 QA 验证分配开发子任务，并限制角色读取项目 wiki 子任务页允许的入口。 |
| `project-extension-development` | 项目扩展开发入口 | active | `../projects/PROJECT_ADAPTER.md` | 1.0.0 | 项目专属开发入口、规则和角色由本机项目胶囊声明，通用层只保留读取、调度和边界机制。 |
| `session-role-marker` | 会话角色标识 | active | `../workflows/session-visible-state.md` | 1.1.1 | 会话首次正式输出、角色切换、最终结论、阶段性结论、交付汇总或正式评审结论展示当前角色、主要职责和工作依据；角色名只能来自角色选择表、项目胶囊扩展角色或统一兜底模板。 |
...
| `tharness-self-check` | Tharness 自检 | active | `../../tools/tharness.py` | 0.6.0 | 需要检查入口、元数据、wiki 索引或交付门控是否存在结构问题。 |
| `workflow-self-check-trigger` | 自检触发规则 | active | `../workflows/development/rules/self-check.md` | 0.6.1 | Tharness 自身改动影响 AIGC 文档、规则、配置、工具或准备交付时。 |
| `index-sync` | Wiki 索引同步 | active | `../../tools/tharness.py` | 0.6.2 | 需要按扫描结果校验或写回 `AIGC/wiki/index.yaml` 页面清单。 |
| `self-check-planner` | 自检命令规划 | active | `../../tools/tharness.py` | 0.6.2 | 需要按变更路径确定本轮应该运行哪些自检命令。 |
