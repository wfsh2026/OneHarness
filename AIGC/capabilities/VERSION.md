...
| Unity UGUI UI 图转施工 | active | `../workflows/unity-ugui-ui-workflow/WORKFLOW.md` | 把 UI 截图、效果图或素材图转成 UI 策划、技术美术资源验收与拼装、UGUI Prefab 施工、Controller 事件接口和截图校准反馈。 |
...
- Unity UGUI UI 工作流只保存跨项目可复用的三角色流程、已切资源验收规则、施工约束和报告模板；具体界面事实、素材路径、项目 UI 规范和运行记录仍写入目标项目适配层。
...
- Unity UGUI UI 工作流可从 `../workflows/INDEX.md` 路由到 `../workflows/unity-ugui-ui-workflow/WORKFLOW.md`，并按输入完整度、UI 策划、技术美术资源验收与拼装、Unity 施工和截图校准规则继续读取。
