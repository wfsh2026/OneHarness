...
title: 项目适配包层
summary: 用本机忽略目录中的项目适配包保存项目事实、项目专属角色、运行记录和角色会话索引。
...
可提交的通用框架只提供角色规则、通用知识和路由；目标项目事实、运行记录和角色会话索引必须保存在本机 `AIGC/project-adapters/{project_id}/` 项目适配包。

## 适用场景

- 同一套通用角色调度规则要服务多个项目。
- 项目需要保存运行记录、角色会话索引、决策、开放问题、项目知识和项目专属角色。
...
- 项目事实不写入可提交的通用框架。
- 运行记录不写入通用 wiki。
- 角色会话索引只写入 `AIGC/project-adapters/{project_id}/runs/sessions/`。
- 通用规则中不硬编码目标项目路径。
