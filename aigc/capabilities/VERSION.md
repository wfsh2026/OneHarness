...
- poe 小助手工具索引可从 `../roles/poe-helper/tools/INDEX.md` 命中；该入口不绑定项目专属工具路径。
...
- `python tools/tharness.py roles-ui` 自动选择本地端口并打开默认浏览器；`--no-open` 禁止自动打开，`--port` 指定端口。
- `python -m unittest tools.test_role_browser` 覆盖数据装配、文档按需读取、路径越界、只读接口、安全响应头、端口和启动参数。
- `python -m unittest tools.test_tharness tools.test_version_feature_scan` 覆盖机器注册源半接入、CLI、空范围、merge、rename、二进制文件、非法引用、重复仓库、输出冲突、dry-run 和结构化证据输出。
